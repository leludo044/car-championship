...
public class AdminCircuit {
	...
	public Response create(final CircuitParams params) {
		...
		if (StringUtils.isNotEmpty(message)) {
			...
			dao.create(circuit);
			response = Response.ok(new WsReturn(circuit.getId(), "Circuit " + circuit.getNom() + " ajouté !")).build();
		}
	...
	public Response update(final CircuitParams params) {
		...
		if (StringUtils.isNotEmpty(message)) {
			...
			if (circuit != null) {
				...
				dao.update(circuit);
				response = Response
						.ok(new WsReturn(Status.OK.getStatusCode(), "Circuit " + circuit.getNom() + " modifié !"))
						.build();
			} else {
				response = Response.status(Status.NOT_FOUND).entity(
						new WsReturn(Status.NOT_FOUND.getStatusCode(), "Circuit #" + params.getId() + " introuvable !"))
						.build();
			}
		}
		dao.close();
	...
	public Response delete(@PathParam("id") final int id) {
		...
		if (circuit != null) {
			dao.delete(circuit);
			response = Response
					.ok(new WsReturn(Status.OK.getStatusCode(), "Circuit " + circuit.getNom() + " supprimé !")).build();
		} else {
			response = Response.status(Status.NOT_FOUND)
					.entity(new WsReturn(Status.NOT_FOUND.getStatusCode(), "Circuit #" + id + " introuvable !"))
					.build();
		}
		dao.close();
		return response;
