...
public class RaceWebService {
	...
	public Response addResult(final ResultParams params) {
		...
		try {
			...
			response = Response.ok(new WsReturn(Status.OK.getStatusCode(),
					"Results for driver " + competitor.getDriver().getName() + " saved !")).build();
			daoFactory.close();
		} catch (final ChampionshipException ce) {
			response = Response.status(ce.status()).entity(new WsReturn(ce.status().getStatusCode(), ce.getMessage()))
					.build();
		}
	...
	public Response updateResult(final ResultParams params) {
		...
		try {
			...
			response = Response.ok(new WsReturn(Status.OK.getStatusCode(),
					"Results for driver " + competitor.getDriver().getName() + " updated !")).build();
			daoFactory.close();
		} catch (final ChampionshipException ce) {
			response = Response.status(ce.status()).entity(new WsReturn(ce.status().getStatusCode(), ce.getMessage()))
					.build();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response removeResult(@PathParam("raceId") final int raceId, @PathParam("driverId") final int driverId,
			@PathParam("raceNumber") final int raceNumber) {
		Response response;
		...
		if (rowCount == 1) {
			response = Response.ok(new WsReturn(Status.OK.getStatusCode(), "Result removed !")).build();
		} else if (rowCount == 0) {
			response = Response.status(Status.BAD_REQUEST)
					.entity(new WsReturn(Status.BAD_REQUEST.getStatusCode(), "No result to remove !")).build();
		} else {
			response = Response.status(Status.BAD_REQUEST)
					.entity(new WsReturn(Status.BAD_REQUEST.getStatusCode(), "To many results removed  !")).build();
		}
		daoFactory.close();
		return response;
	}
	...
	private void checkArrivalPosition(final ResultParams params) throws ChampionshipException {
		if (params.getArrivalPosition() <= 0) {
			throw new ChampionshipException(Status.NOT_ACCEPTABLE, "Incorrect arrival position ! Must be > 0 !");
		} else {
			DaoFactory daoFactory = DataManager.getInstance().getManager();
			...
			if (params.getArrivalPosition() > maxArrivalPosition) {
				throw new ChampionshipException(Status.NOT_ACCEPTABLE,
						String.format("Incorrect arrival position ! Must be <= %s !", maxArrivalPosition));
			}
		}
	}
	...
	private void checkStartingPosition(final int startingPosition) throws ChampionshipException {
		if (startingPosition <= 0) {
			throw new ChampionshipException(Status.NOT_ACCEPTABLE, "Incorrect starting position ! Must be > 0 !");
		}
	...
	private void checkRaceNumber(final int raceNumber) throws ChampionshipException {
		if (raceNumber < 1 || raceNumber > 2) {
			throw new ChampionshipException(Status.NOT_ACCEPTABLE, "Incorrect race number ! Must be 1 or 2 !");
		}
	 ...
	 */
	private void checkAgainstOtherCompetitor(final Integer id, final int raceNumber, final Competitor competitor)
			throws ChampionshipException {
		...
		for (Competitor other : competitors) {
			if (other.getDriver().getId() != competitor.getDriver().getId()) {
				if (other.getStartingPosition() == competitor.getStartingPosition()) {
					throw new ChampionshipException(Status.NOT_ACCEPTABLE, competitor.getDriver().getName()
							+ " has already the same starting position than " + other.getDriver().getName());
				}
				if (other.getArrivalPosition() == competitor.getArrivalPosition()) {
					throw new ChampionshipException(Status.NOT_ACCEPTABLE, competitor.getDriver().getName()
							+ " has already the same arrival position than " + other.getDriver().getName());
				}
			}
		}
	}
	 ...
	 *             Raised exception on error
	 */
	private void checkExistingResult(final Integer id, final int raceNumber, final Competitor competitor)
			throws ChampionshipException {
