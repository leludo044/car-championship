...
public class ChampionshipWebService {
	...
	public Response create(final ChampionshipParams params) {
		...
		if (name == null || name.equals("")) {
			response = Response.status(Status.NOT_ACCEPTABLE)
					.entity(new WsReturn(Status.NOT_ACCEPTABLE.getStatusCode(), "Championship name is missing !"))
					.build();
		} else if (type == null || type.equals("")) {
			response = Response.status(Status.NOT_ACCEPTABLE)
					.entity(new WsReturn(Status.NOT_ACCEPTABLE.getStatusCode(), "Championship type is missing !"))
					.build();
		} else if (mode != 1 && mode != 2) {
			response = Response.status(Status.NOT_ACCEPTABLE)
					.entity(new WsReturn(Status.NOT_ACCEPTABLE.getStatusCode(), "Championship mode is wrong !"))
					.build();
		} else {
			DaoFactory daoFactory = DataManager.getInstance().getManager();
			ChampionshipDao championshipDao = daoFactory.championshipDao();
			...
			response = Response
					.ok(new WsReturn(championship.getId(), "Championship " + championship.getName() + " created !"))
					.build();
			daoFactory.close();
		}
	...
	public Response delete(@PathParam("id") final int id) {
		...
		if (championship != null) {
			...
			response = Response.ok(
					new WsReturn(Status.OK.getStatusCode(), "Championship " + championship.getName() + " deleted !"))
					.build();
		} else {
			response = Response.status(Status.NOT_FOUND)
					.entity(new WsReturn(Status.NOT_FOUND.getStatusCode(), "Championship #" + id + " not found !"))
					.build();
		}
		daoFactory.close();
		return response;
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(@PathParam("id") final Integer id, @PathParam("trackId") final Integer trackId,
			final RaceParams params) {
		Response response;
		DaoFactory daoFactory = DataManager.getInstance().getManager();
		ChampionshipDao championshipDao = daoFactory.championshipDao();
		...
		if (championship != null) {
			...
			if (track != null) {
				...
				championshipDao.update(championship);
				response = Response
						.ok(new WsReturn(championship.lastRace().getId(),
								"Track " + track.getName() + " added to championship " + championship.getName() + "."))
						.build();
			} else {
				response = Response.status(Status.NOT_FOUND)
						.entity(new WsReturn(Status.NOT_FOUND.getStatusCode(), "Track #" + trackId + " not found !"))
						.build();
			}
		} else {
			response = Response.status(Status.NOT_FOUND)
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response remove(@PathParam("id") final Integer id, @PathParam("trackId") final Integer trackId) {
		Response response;
		DaoFactory daoFactory = DataManager.getInstance().getManager();
		ChampionshipDao championshipDao = daoFactory.championshipDao();
		Championship championship = championshipDao.find(id);
		if (championship != null) {
			if (trackId != null) {
				...
				championshipDao.update(championship);
				response = Response.ok(new WsReturn(Status.OK.getStatusCode(), "Track " + race.getTrack().getName()
						+ " deleted from championship " + championship.getName() + ".")).build();
			} else {
				response = Response.status(Status.NOT_FOUND)
						.entity(new WsReturn(Status.NOT_FOUND.getStatusCode(), "Undefined track to delete !")).build();
			}
		} else {
			response = Response.status(Status.NOT_FOUND)
					.entity(new WsReturn(Status.NOT_FOUND.getStatusCode(), "Championship #" + id + " not found !"))
		...
		if (results == null) {
			...
			try {
				...
				for (Object[] concurrent : results) {
					g.writeStartObject();
					g.writeNumberField("piloteId", ((Competitor) concurrent[0]).getDriver().getId());
					...
					g.writeNumberField("arrivalPosition", ((Competitor) concurrent[0]).getArrivalPosition());
					g.writeNumberField("raceNumber", ((Competitor) concurrent[0]).getRaceNumber());
					g.writeBooleanField("pole", ((Competitor) concurrent[0]).getStartingPosition() == 1);
